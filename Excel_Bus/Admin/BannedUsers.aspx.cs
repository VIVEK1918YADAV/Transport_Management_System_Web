...
namespace Excel_Bus
{
    public partial class BannedUsers : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    RegisterAsyncTask(new PageAsyncTask(() => LoadBannedUsers()));
                }
            }
        }

        private async Task LoadBannedUsers(string searchTerm = "")
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // Filter only banned users (Status != 1)
                    var bannedUsers = users.Where(u => u.Status != 1)
                                          .OrderByDescending(u => u.UpdatedAt)
                                          .ToList();

                    // Store in ViewState for search functionality
                    ViewState["AllUsers"] = JsonConvert.SerializeObject(bannedUsers);

                    // Bind to GridView
                    gvUsers.DataSource = bannedUsers;
                    gvUsers.DataBind();
                }
                else
                {
        ...
        private async Task LoadUserDetails(string userId)
        {
            try
            {
                ...
                string endpoint = $"Users/GetUser/{userId}";
                HttpResponseMessage response = await client.GetAsync(endpoint);
        ...
        {
            // Populate all labels with user data
            lblUserName.Text = $"{user.Firstname} {user.Lastname}";
            //lblUsername.Text = !string.IsNullOrEmpty(user.Username) ? $"@{user.Username}" : "N/A";
            lblEmail.Text = user.Email ?? "N/A";
            lblMobile.Text = $"{user.DialCode} {user.Mobile}";
            ...
            lblStatus.Text = user.Status == 1 ?
                "<span class='status-badge status-active'>Active</span>" :
                "<span class='status-badge status-banned'>Banned</span>";

            lblBanReason.Text = !string.IsNullOrEmpty(user.BanReason) ? user.BanReason : "No reason provided";

            lblEmailVerified.Text = user.Ev == 1 ?
                "<span class='verified-yes'>Yes</span>" :
                "<span class='verified-no'>No</span>";
            lblMobileVerified.Text = user.Sv == 1 ?
                "<span class='verified-yes'>Yes</span>" :
                "<span class='verified-no'>No</span>";
            lblProfileComplete.Text = user.ProfileComplete == 1 ?
                "<span class='verified-yes'>Yes</span>" :
                "<span class='verified-no'>No</span>";
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(searchTerm))
            {
                ...
                else
                {
                    ...
                    RegisterAsyncTask(new PageAsyncTask(() => LoadBannedUsers(searchTerm)));
                }
            }
            else
            {
        ...
        // Helper method to get ban reason with default text
        protected string GetBanReason(object banReason)
        {
            if (banReason == null || string.IsNullOrEmpty(banReason.ToString()))
                return "No reason provided";

            return banReason.ToString();
        }
