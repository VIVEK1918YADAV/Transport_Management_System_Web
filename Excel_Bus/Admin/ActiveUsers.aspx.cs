...
namespace Excel_Bus
{
    // User model class - define at namespace level
    public class User
    {
        [JsonProperty("id")]
        ...
        public string VerCodeSendAt { get; set; }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    RegisterAsyncTask(new PageAsyncTask(() => LoadActiveUsers()));
                }
            }
        }

        private async Task LoadActiveUsers(string searchTerm = "")
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var activeUsers = users.Where(u => u.Status == 1)
                                          .OrderByDescending(u => u.CreatedAt)
                                          .ToList();

                    // Store in ViewState for search functionality
                    ViewState["AllUsers"] = JsonConvert.SerializeObject(activeUsers);

                    // Bind to GridView
                    gvUsers.DataSource = activeUsers;
                    gvUsers.DataBind();
                }
                else
                {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(searchTerm))
            {
                ...
                else
                {
                    ...
                    RegisterAsyncTask(new PageAsyncTask(() => LoadActiveUsers(searchTerm)));
                }
            }
            else
            {
