...
namespace Excel_Bus
{
    public partial class BookedTickets : System.Web.UI.Page
    {
        ...
        private List<BookedTicketViewModel> AllTicketsData
        {
            get
            {
                if (ViewState["AllTicketsData"] != null)
                    return JsonConvert.DeserializeObject<List<BookedTicketViewModel>>(ViewState["AllTicketsData"].ToString());
                return new List<BookedTicketViewModel>();
            }
        ...
        private async Task LoadBookedTickets()
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        var allBookings = JsonConvert.DeserializeObject<List<BookedTicketApiModel>>(jsonResponse);
                        ...
                        if (allBookings != null && allBookings.Count > 0)
                        {
                            ...
                            {
                                var ticketList = bookedTickets.Select(b => new BookedTicketViewModel
                                {
                                    Id = b.BookedId,
                                    UserId = b.UserId ?? 0,
        ...
        private void SetNoDataState()
        {
            ...
            {
                rptBookedTickets.DataSource = new List<BookedTicketViewModel>();
                rptBookedTickets.DataBind();
            }
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (!string.IsNullOrEmpty(jsonResponse) && jsonResponse != "[]")
                    {
                        ...
                        if (allBookings != null && allBookings.Count > 0)
                        {
                            ...
                            {
                                var ticketList = finalTickets.Select(b => new BookedTicketViewModel
                                {
                                    Id = b.BookedId,
                                    UserId = b.UserId ?? 0,
                                    UserName = b.UserName ?? "N/A",
    ...
    // API Model
    public class BookedTicketApiModel
    {
