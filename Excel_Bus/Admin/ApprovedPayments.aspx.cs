...
namespace Excel_Bus
{
    public partial class ApprovedPayments : System.Web.UI.Page
    {
        private static readonly HttpClient client;
        private static readonly string apiUrl;

        static ApprovedPayments()
        {
        ...
        private List<ApprovedTransactionViewModel> AllPaymentsData
        {
            get
            {
                if (ViewState["AllPaymentsData"] != null)
                    return JsonConvert.DeserializeObject<List<ApprovedTransactionViewModel>>(ViewState["AllPaymentsData"].ToString());
                return new List<ApprovedTransactionViewModel>();
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                RegisterAsyncTask(new PageAsyncTask(() => LoadApprovedPayments()));
            }
        }

        private async Task LoadApprovedPayments(string searchTerm = "", string dateRange = "")
        {
            try
            {
                ...
                List<ApprovedTransactionDto> transactions = JsonConvert.DeserializeObject<List<ApprovedTransactionDto>>(transactionJson);
                ...
                List<ApprovedUserDto> users = JsonConvert.DeserializeObject<List<ApprovedUserDto>>(userJson);

                // Create dictionary for quick user lookup
                var userDict = users.ToDictionary(u => u.Id, u => u);

                // Filter for SUCCESS/APPROVED payments (status = "Success")
                var approvedPayments = transactions
                    .Where(t => t.Status != null && t.Status.Equals("Success", StringComparison.OrdinalIgnoreCase))
                    .Select(t => new ApprovedTransactionViewModel
                    {
                        Id = t.TrxId,
                        UserId = t.UserId,
                        Amount = t.Amount,
                ...
                {
                    approvedPayments = approvedPayments.Where(t =>
                ...
                AllPaymentsData = approvedPayments;
                ...
                pnlNoData.Visible = approvedPayments.Count == 0;
            }
            catch (Exception ex)
            {
        ...
        private void BindPagedData()
        {
            try
            {
                ...
                {
                    rptPayments.DataSource = new List<ApprovedTransactionViewModel>();
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            RegisterAsyncTask(new PageAsyncTask(() => LoadApprovedPayments(searchTerm, dateRange)));
        }
    ...
    // DTOs for Approved Payments page
    ...
    public class ApprovedTransactionViewModel
    {
        public int Id { get; set; }
        public int UserId { get; set; }
        public decimal Amount { get; set; }
