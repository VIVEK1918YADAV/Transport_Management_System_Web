...
namespace Excel_Bus
{
    public partial class RejectedTickets : System.Web.UI.Page
    {
        ...
        private List<RejectedTicketViewModel> AllTicketsData
        {
            get
            {
                if (ViewState["AllTicketsData"] != null)
                    return JsonConvert.DeserializeObject<List<RejectedTicketViewModel>>(ViewState["AllTicketsData"].ToString());
                return new List<RejectedTicketViewModel>();
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                System.Diagnostics.Debug.WriteLine("=== RejectedTickets Page Load ===");
                RegisterAsyncTask(new PageAsyncTask(() => LoadRejectedTickets()));
            }
        }

        private async Task LoadRejectedTickets()
        {
            try
            {
                System.Diagnostics.Debug.WriteLine("LoadRejectedTickets started");
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        var allBookings = JsonConvert.DeserializeObject<List<RejectedTicketApiModel>>(jsonResponse);

                        if (allBookings != null && allBookings.Count > 0)
                        {
                            var rejectedTickets = allBookings
                                .Where(b => !string.IsNullOrEmpty(b.Status) &&
                                           b.Status.Equals("Rejected", StringComparison.OrdinalIgnoreCase))
                                .ToList();

                            if (rejectedTickets.Count > 0)
                            {
                                var ticketList = rejectedTickets.Select(b => new RejectedTicketViewModel
                                {
                                    Id = b.BookedId,
                                    UserId = b.UserId ?? 0,
                                ...
                                if (litTotalRejected != null)
                                {
                                    litTotalRejected.Text = ticketList.Count.ToString();
                                }
                ...
                {
                    SetNoDataState();
                    ShowMessage("Failed to load rejected tickets.", "error");
                }
            }
            catch (Exception ex)
            {
                ...
                SetNoDataState();
                ShowMessage("Error loading rejected tickets. Please try again.", "error");
            }
        }
        ...
        private void BindPagedData()
        {
            try
            {
                ...
                // Bind to repeater
                rptRejectedTickets.DataSource = pagedTickets;
                rptRejectedTickets.DataBind();
        ...
        {
            if (rptRejectedTickets != null)
            {
                rptRejectedTickets.DataSource = new List<RejectedTicketViewModel>();
                rptRejectedTickets.DataBind();
            }
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (!string.IsNullOrEmpty(jsonResponse) && jsonResponse != "[]")
                    {
                        ...
                        if (allBookings != null && allBookings.Count > 0)
                        {
                            // Filter by status first (Rejected only)
                            var rejectedTickets = allBookings
                                .Where(b => !string.IsNullOrEmpty(b.Status) &&
                                           b.Status.Equals("Rejected", StringComparison.OrdinalIgnoreCase))
                                .ToList();
                            ...
                            // Apply filters
                            var filteredTickets = rejectedTickets.AsEnumerable();
                            ...
                            {
                                var ticketList = finalTickets.Select(b => new RejectedTicketViewModel
                                {
                                    Id = b.BookedId,
                                    UserId = b.UserId ?? 0,
                                    UserName = b.UserName ?? "N/A",
    ...
    // View Model
    public class RejectedTicketViewModel
    {
    ...
    // API Model
    public class RejectedTicketApiModel
    {
