...
namespace Excel_Bus
{
    public partial class SuccessfulPayments : System.Web.UI.Page
    {
        ...
        private List<SuccessfulTransactionViewModel> AllPaymentsData
        {
            get
            {
                if (ViewState["AllPaymentsData"] != null)
                    return JsonConvert.DeserializeObject<List<SuccessfulTransactionViewModel>>(ViewState["AllPaymentsData"].ToString());
                return new List<SuccessfulTransactionViewModel>();
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                RegisterAsyncTask(new PageAsyncTask(() => LoadSuccessfulPayments()));
            }
        }

        private async Task LoadSuccessfulPayments(string searchTerm = "", string dateRange = "")
        {
            try
            {
                ...
                List<SuccessfulTransactionDto> transactions = JsonConvert.DeserializeObject<List<SuccessfulTransactionDto>>(transactionJson);
                ...
                List<SuccessfulUserDto> users = JsonConvert.DeserializeObject<List<SuccessfulUserDto>>(userJson);

                // Create dictionary for quick user lookup
                var userDict = users.ToDictionary(u => u.Id, u => u);

                // Filter for SUCCESSFUL payments (status = "Success")
                var successfulPayments = transactions
                    .Where(t => t.Status != null && t.Status.Equals("Success", StringComparison.OrdinalIgnoreCase))
                    .Select(t => new SuccessfulTransactionViewModel
                    {
                        Id = t.TrxId,
                        UserId = t.UserId,
                        Amount = t.Amount,
                ...
                {
                    successfulPayments = successfulPayments.Where(t =>
                ...
                AllPaymentsData = successfulPayments;
                ...
                pnlNoData.Visible = successfulPayments.Count == 0;
            }
            catch (Exception ex)
            {
        ...
        private void BindPagedData()
        {
            try
            {
                ...
                {
                    rptPayments.DataSource = new List<SuccessfulTransactionViewModel>();
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            RegisterAsyncTask(new PageAsyncTask(() => LoadSuccessfulPayments(searchTerm, dateRange)));
        }
    ...
    // DTOs for Successful Payments page
    ...
    public class SuccessfulTransactionViewModel
    {
        public int Id { get; set; }
        public int UserId { get; set; }
        public decimal Amount { get; set; }
