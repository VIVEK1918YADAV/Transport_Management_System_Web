...
namespace Excel_Bus
{
    public partial class RejectedPayments : System.Web.UI.Page
    {
        private static readonly HttpClient client;
        private static readonly string apiUrl;

        static RejectedPayments()
        {
        ...
        private List<RejectedTransactionViewModel> AllPaymentsData
        {
            get
            {
                if (ViewState["AllPaymentsData"] != null)
                    return JsonConvert.DeserializeObject<List<RejectedTransactionViewModel>>(ViewState["AllPaymentsData"].ToString());
                return new List<RejectedTransactionViewModel>();
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                RegisterAsyncTask(new PageAsyncTask(() => LoadRejectedPayments()));
            }
        }

        private async Task LoadRejectedPayments(string searchTerm = "", string dateRange = "")
        {
            try
            {
                ...
                List<RejectedTransactionDto> transactions = JsonConvert.DeserializeObject<List<RejectedTransactionDto>>(transactionJson);
                ...
                List<RejectedUserDto> users = JsonConvert.DeserializeObject<List<RejectedUserDto>>(userJson);

                // Create dictionary for quick user lookup
                var userDict = users.ToDictionary(u => u.Id, u => u);

                // Filter for rejected payments (status = "Rejected")
                var rejectedPayments = transactions
                    .Where(t => t.Status != null && t.Status.Equals("Rejected", StringComparison.OrdinalIgnoreCase))
                    .Select(t => new RejectedTransactionViewModel
                    {
                        Id = t.TrxId,
                        UserId = t.UserId,
                        Amount = t.Amount,
                ...
                {
                    rejectedPayments = rejectedPayments.Where(t =>
                ...
                AllPaymentsData = rejectedPayments;
                ...
                pnlNoData.Visible = rejectedPayments.Count == 0;
            }
            catch (Exception ex)
            {
        ...
        private void BindPagedData()
        {
            try
            {
                ...
                {
                    rptPayments.DataSource = new List<RejectedTransactionViewModel>();
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            RegisterAsyncTask(new PageAsyncTask(() => LoadRejectedPayments(searchTerm, dateRange)));
        }
        ...
        {
            if (status == null) return "status-rejected";
    ...
    // DTOs for Rejected Payments page
    ...
    public class RejectedTransactionViewModel
    {
        public int Id { get; set; }
        public int UserId { get; set; }
        public decimal Amount { get; set; }
